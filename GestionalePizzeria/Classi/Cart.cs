...
namespace GestionalePizzeria
{
    public static class Cart
    {
        ...
        public static int GetCartQuantitybyID(int itemId)
        {
            List<Product> cartList = HttpContext.Current.Session["Carrello"] as List<Product>;
            if (cartList == null) { return 0; }
            else { return cartList.Count(elem => elem.IdProduct == itemId); }
        }
        ...
        public static decimal GetCartTotal()
        {
            ...
            else
            {
                ...
                foreach (Product item in cartList) { total += item.Prezzo; } // Calcola il totale sommando i prezzi di tutti i prodotti nel carrello
        ...
        public static void AddToCart(int itemId)
        {
            ...
            // Cerca il prodotto nel db in base all'ID
            Product item = new ModelDbContext().Products.Find(itemId);
        ...
        public static void RemoveFromCart(int itemId)
        {
            ...
            Product item = cartList.FirstOrDefault(elem => elem.IdProduct == itemId);
